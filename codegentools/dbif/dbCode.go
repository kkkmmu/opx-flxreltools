...
func (obj *ObjectInfoJson) WriteStoreObjectInDBFcn(str *ast.StructType, fd *os.File, attrMap []ObjectMemberAndInfo, objMap map[string]ObjectInfoJson) {
	...
	if obj.AutoCreate || obj.AutoDiscover {
		...
		lines = append(lines,
			`_, err := dbHdl.Do("HMSET", redis.Args{}.Add("Default#"+obj.GetKey()).AddFlat(obj)...)
		if err != nil {
...
func (obj *ObjectInfoJson) WriteGetAllObjFromDbFcn(str *ast.StructType, fd *os.File, attrMap []ObjectMemberAndInfo, objMap map[string]ObjectInfoJson) {
	...
	lines = append(lines,
		...
		if strings.HasPrefix(keys[idx], "Default#") {
		    continue
		}
...
func (obj *ObjectInfoJson) WriteSortObjListFcn(str *ast.StructType, fd *os.File, attrMap []ObjectMemberAndInfo, objMap map[string]ObjectInfoJson) {
	...
	if key != "" {
		...
		lines = append(lines, "func (a "+obj.ObjName+"s) Less(i, j int) bool {\n")
		lines = append(lines, "key1 := a[i].GetKey()\n")
		lines = append(lines, "key2 := a[j].GetKey()\n")
		lines = append(lines, "normalizedKey1 := alphaNumSort.Normalize(key1)\n")
		lines = append(lines, "normalizedKey2 := alphaNumSort.Normalize(key2)\n")
		lines = append(lines, "return (alphaNumSort.IsLess(normalizedKey1, normalizedKey2))\n")
		lines = append(lines, "}\n")
