...
class FlexObject(object) :
    ...
    def __init__ (self,     # Yours truly
        ...
        with open(attrFile) as fileHdl:
            ...
            for attrName, tmpInfo in attrDict.iteritems():
                ...
                elif tmpDict['isDefaultSet'] == 'True':
    ...
    def createTblPrintAllMethod(self, fileHdl):
        tabs = self.TAB
        lines = []
        lines.append("\n"+ tabs + "def print" + self.name + "s(self, addHeader=True, brief=None, writeToFile=False):\n")
        tabs = tabs + self.TAB
        ...
        lines.append(tabs + "result = self.tblFormatObject(\'%s\', header, rows)\n\n" %(self.name))
        lines.append(tabs + "if not writeToFile:\n")
        lines.append(tabs + self.TAB + "print result\n")
        lines.append(tabs + "elif self.fPath:\n")
        lines.append(tabs + self.TAB + "with open(self.fPath, 'w') as textFile:\n")
        lines.append(tabs + self.TAB + self.TAB + "textFile.write(result)\n")
        fileHdl.writelines(lines)
    ...
    def createTblPrintMethod(self, fileHdl):
        ...
        lines.append("\n"+ tabs + "def print" + self.name + "(self, %s addHeader=True, brief=None, writeToFile=False):\n" %argStr)
        tabs = tabs + self.TAB
        ...
        lines.append(tabs + self.TAB + "rows.append(values)\n")
        lines.append(tabs + self.TAB + "result = self.tblFormatObject(\'%s\', header, rows)\n\n" %(self.name))

        lines.append(tabs + "else:\n")
        lines.append(tabs + self.TAB + "result = rawobj.content\n\n")
        lines.append(tabs + "if not writeToFile:\n")
        lines.append(tabs + self.TAB + "print result\n")
        lines.append(tabs + "elif self.fPath:\n")
        lines.append(tabs + self.TAB + "with open(self.fPath, 'w') as textFile:\n")
    ...
    def createCombinedTblPrintAllMethod(self, fileHdl, cfgObjName, cfgObjAttrs):
        tabs = self.TAB
        lines = []
        lines.append("\n"+ tabs + "def printCombined" + self.name + "s(self, addHeader=True, brief=None, writeToFile=False):\n")
        tabs = tabs + self.TAB
        ...
        cfgAttrList = self._getCfgAttList(cfgObjName)
        if cfgAttrList:
            lines.append(tabs + self.TAB + "r = self.swtch.get" + cfgObjName + "(")
            argStr = ''
            for (attr, attrInfo) in cfgAttrList:
                if attrInfo['isKey'] == 'True':
    ...
    def _getCfgAttList(self, cfgObjName):
        path = self.attrFile.split("/")
        filepath = "/".join(path[:-1]+[cfgObjName+"Members.json"])
        if os.path.exists(filepath):
            with open(filepath) as fileHdl:
                attrDict = json.load(fileHdl)
                attrList = [None] *len(attrDict)
                keysList = []
                dfltAttrList = []
                ...
                return keysList + [x for x in attrList if x!= None] + dfltAttrList
